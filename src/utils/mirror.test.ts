...
describe('mirrorBeadId — NODE', () => {
  ...
  it('нечётный ряд: c ↔ width-2-c (крайние узлы имеют пару)', () => {
  ...
  it('центральный узел нечётного ряда — сам себе пара', () => {
    expect(mirrorBeadId('node-1-2', W, IT)).toBe('node-1-2');
  });

  it('узел за пределами сетки → null', () => {
    // чётный ряд: c=6 → mc=-1
...
describe('mirrorBeadId — верхняя/нижняя кромка', () => {
  it('top-link: колонка width-2-c, индекс internalTop+1-i', () => {
  ...
  it('bottom-link зеркалится, когда задан internalBottom', () => {
    expect(mirrorBeadId('span-edge-bottom-link-1-bead-1', W, IT, IB)).toBe(
      'span-edge-bottom-link-3-bead-2',
    );
  });

  it('bottom-link без internalBottom → null', () => {
    expect(mirrorBeadId('span-edge-bottom-link-1-bead-1', W, IT)).toBeNull();
  });
});

describe('mirrorBeadId — вертикальные грани (vert-edge)', () => {
  it('чётный ряд: колонка width-1-c, сторона left↔right', () => {
