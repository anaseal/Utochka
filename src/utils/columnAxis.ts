...
// от неё осталось. Раньше её считали как половину максимального x уцелевших
// узлов: при активном Taper с ненулевым depth срез держится по всему полотну,
// максимальный x уменьшался, и линия оси съезжала вбок — хотя зеркалит
// редактор по-прежнему по полной сетке (mirrorCol в mirror.ts), т.е. линия
// показывала не ту ось, вокруг которой реально работает Mirror Mode.
//
