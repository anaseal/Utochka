...
// срез уводил бы правый край на полколонки от целевой линии. Симптом такой: у
// края остаются узлы, к которым не подходит ни один пролёт (оба соседа в
// соседних рядах срезаны) — бисерина, которую физически не на чем сплести. При
// симметричном Edge Extension (дефолт) формула даёт ровно тот же результат,
// что и общий срез на обе стороны.
//
