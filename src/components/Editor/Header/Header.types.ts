...
export interface SharedHeaderProps {
  ...
  // Свёрнутый хедер — настройка ландшафтного телефона (см. useAppSettings и
  // Header.css): в строке остаётся только то, чем работают прямо сейчас, всё
  // остальное прячется до разворота. На других ширинах флаг ничего не меняет,
  // но кнопка-стрелка рендерится всегда — показывает её тот же медиа-запрос.
