  ...
  return (
    <header className={`header${weaveMode ? ' header--weave' : ''}${headerCollapsed ? ' header--collapsed' : ''}`}>
      <nav className="header__nav">
        ...
        {/* Только выбор техники. Правая панель (Decor/Grid) — отдельная кнопка
            строки на всех ширинах, а не пункт здесь: в попапе «что я плету» до
            подвесок и сетки пришлось бы докликиваться через меню — на телефоне,
            где панель нужнее всего. */}
        ...
        {/* Без grid-controls--stacked: этот класс задаёт .grid-controls__label
            { min-width: 4.4em } — выравнивание пары лейблов друг под другом
            (Width/Height в GridSidebar), а одиночному Zoom такой min-width
            просто раздувает подпись "ZOOM" шире нужного.
