...
interface CanvasRulersProps {
  ...
  // CanvasView, чтобы сама панель осталась на своём месте, а не наехала на
  // крайние бисерины нечётных рядов (см. spec.md). Ось зеркала и номера
...
export const CanvasRulers = ({ beads, topSpan, bottomSpan, rowSpanOverrides, onRowSpanChange, width, topEdgeEnabled, bottomEdgeEnabled, spanControlsExpanded, gutterShiftX, labelTransform }: CanvasRulersProps) => {
  ...
  // Один узел на чётный ряд — координата Y для номера строки. Берём первый
  // попавшийся узел ряда, а не строго col===0: при активном Taper (сужение
  // концов, см. spec.md) у крайних рядов колонки 0 может не быть вовсе, а для
  // позиционирования подписи важен только y.
